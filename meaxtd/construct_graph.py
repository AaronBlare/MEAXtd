...
def find_delayed_spikes(data):
    num_channels = data.stream.shape[1]
    burst_id = 110
    curr_burst = list(data.bursts[burst_id])
    ...
    curr_channels = list(set([curr_burst[interval_id].data['signal_id'] for interval_id in range(0, len(curr_burst))]))
    curr_channels.sort()
    sampling_rate = (data.time[1] - data.time[0]) * 1000  # in ms
    if sampling_rate >= 0.05:  # frame size = 0.05 ms
        frame_size = 1
    else:
    ...
    c_ij_dict = {'Channel 1': [], 'Channel 2': [], 'Num delayed spikes': [], 'C_ij_max': [], 'tau': []}
